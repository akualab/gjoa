...
func computeNetAlpha(t *testing.T) {
	...
	// t=0, entry state, first model.
	chain[0].α.Set(0.0, 0, 0)
	printLog2(t, "net_alpha", 0, 0, 0, chain[0].α)

	// t=0, entry state, after first model.
	for q := 1; q < len(chain); q++ {
		n := chain[q]
		pn := chain[q-1]
...
func newXHMM(name string, id uint16, a *narray.NArray, b []model.Scorer) *xhmm {
...
func initXChainFB() {
	...
	hmm0 := newXHMM("model 0", 0, narray.New(nstates[0], nstates[0]),
		[]model.Scorer{nil, newScorer(0, 1), newScorer(0, 2), newScorer(0, 3), nil})

	hmm1 := newXHMM("model 0", 0, narray.New(nstates[1], nstates[1]),
	...
	compositeHMM = newXChain(hmm0, hmm1)
}
